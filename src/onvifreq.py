...
class OnvifRequest:
    ...
    def absolute_move(self, x, y):
            ...
            <Position>
                <PanTilt x="{x}" y="{y}" \
                    xmlns="http://www.onvif.org/ver10/schema" \
                        space="http://www.onvif.org/ver10/tptz/PanTiltSpaces/PositionGenericSpace"></PanTilt>
                <Zoom x="0.0" \
                    xmlns="http://www.onvif.org/ver10/schema" \
                        space="http://www.onvif.org/ver10/tptz/ZoomSpaces/PositionGenericSpace"></Zoom>
            </Position>
            <Speed>
                <PanTilt x="1" y="1" \
                    xmlns="http://www.onvif.org/ver10/schema" \
        ...
        """
        return self.request(
            self.username, self.password, command.format(x=x, y=y))
    ...
    def request(self, username, password, command):
        ...
        sha1 = hashlib.sha1()
        sha1.update(
            raw_nonce + created.encode("utf8") + password.encode("utf8"))
